...
let editIndex = null;
let deleteTarget = { module: null, index: null };
...
const FIELDS = {
  residents: [
    ...
    { id: "alias", label: "Alexis", type: "text", placeholder: "Alexis" },
  ...
  fields.forEach((f) => {
    ...
    if (f.type === "select") {
      const opts = f.options
        .map(
          (o) =>
            `<option value="${o}" ${record[f.id] === o ? "selected" : ""}>${o}</option>`,
        )
        .join("");
      input = `<select id="field-${f.id}"><option value="">— Select —</option>${opts}</select>`;
    } else {
      input = `<input id="field-${f.id}" type="${f.type}" placeholder="${f.placeholder || ""}" value="${record[f.id] || ""}">`;
    }
...
function renderResidents(data = null) {
  ...
  if (!rows.length) {
    tb.innerHTML = `<tr><td colspan="6"><div class="empty-state"><div class="icon">🏘</div><p>No residents yet. Add the first one!</p></div></td></tr>`;
    return;
  }
  tb.innerHTML = rows
    .map(
...
function renderWelcoming(data = null) {
  ...
  if (!rows.length) {
    tb.innerHTML = `<tr><td colspan="5"><div class="empty-state"><div class="icon">🤝</div><p>No welcoming members yet.</p></div></td></tr>`;
    return;
  }
  tb.innerHTML = rows
    .map(
...
function renderOfficers(data = null) {
  ...
  if (!rows.length) {
    tb.innerHTML = `<tr><td colspan="7"><div class="empty-state"><div class="icon">⭐</div><p>No officers yet.</p></div></td></tr>`;
    return;
  }
  tb.innerHTML = rows
    .map(
...
function renderFormer(data = null) {
  ...
  if (!rows.length) {
    tb.innerHTML = `<tr><td colspan="7"><div class="empty-state"><div class="icon">📜</div><p>No former officers yet.</p></div></td></tr>`;
    return;
  }
  tb.innerHTML = rows
    .map(
...
function filterTable(module, query) {
  ...
  const filtered = DB[module].filter((r) =>
...
function updateStats() {
  ...
  ["r", "w", "o", "f"].forEach((k, i) => {
    ...
    document.getElementById(
      "stat-" + ["residents", "welcoming", "officers", "former"][i],
    ).textContent = c;
    document.getElementById(
      "badge-" + ["residents", "welcoming", "officers", "former"][i],
    ).textContent = c;
